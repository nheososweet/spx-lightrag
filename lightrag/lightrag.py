...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        self.entities_vdb: BaseVectorStorage = self.vector_db_storage_cls(  # type: ignore
            ...
            workspace=self.workspace,
            embedding_func=self.embedding_func,
            meta_fields={"entity_name", "source_id", "content", "file_path", "isDeleted"},
        )
        self.relationships_vdb: BaseVectorStorage = self.vector_db_storage_cls(  # type: ignore
            namespace=NameSpace.VECTOR_STORE_RELATIONSHIPS,
            workspace=self.workspace,
            embedding_func=self.embedding_func,
            meta_fields={"src_id", "tgt_id", "source_id", "content", "file_path", "isDeleted"},
        )
    ...
    async def _asoft_delete_doc_internal(
        self, doc_id: str
    ) -> DeletionResult:
        """Internal method for soft deleting a document without locking pipeline status."""
        try:
            ...
            if doc_status_data.get("isDeleted", False):
                return DeletionResult(
                    status="success",
                    doc_id=doc_id,
                    message=f"Document {doc_id} is already deleted.",
                    status_code=200,
            ...
            # 2. Soft delete document and status
            await self.full_docs.soft_delete([doc_id])
            await self.doc_status.soft_delete([doc_id])

            # 3. Soft delete chunks
            chunk_ids = set(doc_status_data.get("chunks_list", []))
            if chunk_ids:
                chunk_id_list = list(chunk_ids)
                await self.text_chunks.soft_delete(chunk_id_list)
                await self.chunks_vdb.soft_delete(chunk_id_list)
                
                # 4. Analyze entities and relationships to soft delete
                # Only delete entities/relations if they are orphaned (no other active source chunks)
                
                # Get entities generated by this doc
                doc_entities_data = await self.full_entities.get_by_id(doc_id)
                entities_to_check = set()
                if doc_entities_data and "entity_names" in doc_entities_data:
                    entities_to_check.update(doc_entities_data["entity_names"])
                
                orphaned_entities = []
                for entity_name in entities_to_check:
                    # Check sources in entity_chunks
                    stored_chunks = await self.entity_chunks.get_by_id(entity_name)
                    if not stored_chunks:
                        continue
                        
                    all_source_chunks = set(stored_chunks.get("chunk_ids", []))
                    # Remove chunks of current document
                    remaining_chunks = subtract_source_ids(list(all_source_chunks), chunk_ids)
                    
                    if not remaining_chunks:
                        # No other chunks -> Orphaned
                        orphaned_entities.append(entity_name)
                    else:
                        # Check if remaining chunks belong to ACTIVE documents
                        active_chunks_data = await self.text_chunks.get_by_ids(remaining_chunks)
                        # Filter out Nones
                        active_chunks_data = [c for c in active_chunks_data if c]
                        
                        if not active_chunks_data:
                             # All remaining chunks are deleted (physically or soft)
                             orphaned_entities.append(entity_name)
                        else:
                             # We have at least one active source chunk
                             pass # Entity remains active

                if orphaned_entities:
                    logger.info(f"Soft deleting {len(orphaned_entities)} orphaned entities")
                    await self.chunk_entity_relation_graph.soft_remove_nodes(orphaned_entities)
                    
                    # entities_vdb uses hashed IDs
                    entity_vdb_ids = [compute_mdhash_id(name, prefix="ent-") for name in orphaned_entities]
                    await self.entities_vdb.soft_delete(entity_vdb_ids)
                    
                    await self.entity_chunks.soft_delete(orphaned_entities)
                
                # Get relations generated by this doc
                doc_relations_data = await self.full_relations.get_by_id(doc_id)
                relations_to_check = set()
                if doc_relations_data and "relation_pairs" in doc_relations_data:
                    relations_to_check.update([tuple(pair) for pair in doc_relations_data["relation_pairs"]])
                
                orphaned_relations = [] # List of tuples
                for src, tgt in relations_to_check:
                    edge_key = make_relation_chunk_key(src, tgt)
                    stored_chunks = await self.relation_chunks.get_by_id(edge_key)
                    if not stored_chunks:
                        continue
                ...
                if orphaned_relations:
                    logger.info(f"Soft deleting {len(orphaned_relations)} orphaned relations")
                    await self.chunk_entity_relation_graph.soft_remove_edges(orphaned_relations)
                    rel_ids = [compute_mdhash_id(src + tgt, prefix="rel-") for src, tgt in orphaned_relations]
                    await self.relationships_vdb.soft_delete(rel_ids)
                    
                    # Update relation_chunks with soft delete
                    orphaned_relation_keys = [make_relation_chunk_key(src, tgt) for src, tgt in orphaned_relations]
                    await self.relation_chunks.soft_delete(orphaned_relation_keys)
            
            return DeletionResult(
                status="success",
                doc_id=doc_id,
                message=f"Soft deleted document {doc_id}",
                status_code=200,
                file_path=file_path,
            )

        except Exception as e:
            logger.error(f"Soft deletion failed for {doc_id}: {e}")
            return DeletionResult(
                status="fail",
                doc_id=doc_id,
                message=str(e),
    ...
    async def asoft_delete_by_doc_id(
        self, doc_id: str
    ) -> DeletionResult:
        """Soft delete a document and its associated data (chunks, entities, relationships).

        This method marks the document and its components as deleted (isDeleted=True)
        instead of physically removing them. This allows for potential restoration.

        Args:
            doc_id (str): The unique identifier of the document to be soft-deleted.

        Returns:
            DeletionResult: Outcome of the operation.
        """
        # Get pipeline status shared data and lock for validation
        pipeline_status = await get_namespace_data(
            "pipeline_status", workspace=self.workspace
        ...
        async with pipeline_status_lock:
            if not pipeline_status.get("busy", False):
                ...
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": "Single document soft deletion",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": 1,
                        "batchs": 1,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting soft deletion for document: {doc_id}",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting soft deletion for document: {doc_id}"
                ]
            else:
                 # Pipeline already busy
                return DeletionResult(
                    status="not_allowed",
                    doc_id=doc_id,
                    message=f"Deletion not allowed: pipeline is busy",
                    status_code=403,
                    file_path=None,
                )
        ...
        try:
            async with pipeline_status_lock:
                logger.info(f"Soft deleting document {doc_id}")
                pipeline_status["latest_message"] = f"Soft deleting document {doc_id}"

            result = await self._asoft_delete_doc_internal(doc_id)
            ...
            if we_acquired_pipeline:
                 async with pipeline_status_lock:
                    ...
                    pipeline_status["cancellation_requested"] = False
                    pipeline_status["cancellation_requested"] = False
                    logger.info(f"Soft deletion process completed for {doc_id}")

    async def _aactive_doc_internal(
        self, doc_id: str
    ) -> DeletionResult:
         """Internal method for reactivating a document without locking pipeline status."""
         try:
            # 1. Get doc status
            # Use include_deleted=True to find it if it was deleted
            # JsonDocStatusStorage.get_by_id does NOT check isDeleted by default?
            # Let's check JsonDocStatusStorage implementation.
            # It calls BaseKVStorage.get_by_id which calls self._data.get(id).
            # Wait, JsonDocStatusStorage inherits from DocStatusStorage.
            # I implemented soft_delete in JsonDocStatusStorage but didn't change get_by_id in JsonKVStorage to filter?
            # I changed get_by_id in JsonKV to accept include_deleted=False (and filter).
            # Does doc_status use JsonKV.get_by_id?
            # self.doc_status is initialized as DocStatusStorage.
            # If it is JsonDocStatusStorage, it overrides methods?
            # JsonDocStatusStorage implementation of get_by_id (if inherited from JsonKVStorage) now filters.
            # I need to use include_deleted=True.
            # But BaseKVStorage.get_by_id signature was NOT changed in base.py?
            # I checked base.py. I didn't change abstract signature.
            # But I changed JsonKVStorage.get_by_id signature.
            pass

            # Since I didn't change base interface, I can't pass include_deleted=True safely if typed as BaseKVStorage?
            # self.doc_status is typed as DocStatusStorage.
            # DocStatusStorage doesn't have get_by_id defined with include_deleted?
            # It inherits BaseKVStorage.
            # But implementations (JsonKV) have it.
            # Python call will work if method accepts it.
            # Let's assume standard call and if None, try explicit mechanism?
            # Actually, `get_by_id` in `JsonKVStorage` defaults `include_deleted` to False. 
            # So `get_by_id(doc_id)` will return None if deleted.
            # I must use `get_by_id(doc_id, include_deleted=True)` if supported.
            # Or use `get_by_ids([doc_id], include_deleted=True)`.
            
            # Since I don't know if doc_status storage supports include_deleted in get_by_id (interface issue),
            # I'll try calling it with argument. If fails, catch TypeError.
            
            try:
                doc_status_data = await self.doc_status.get_by_id(doc_id, include_deleted=True)
            except TypeError:
                 doc_status_data = await self.doc_status.get_by_id(doc_id)

            if not doc_status_data:
                logger.warning(f"Document {doc_id} not found for reactivation")
                return DeletionResult(
                    status="not_found",
                    doc_id=doc_id,
                    message=f"Document {doc_id} not found.",
            ...
            if not doc_status_data.get("isDeleted", False):
                 return DeletionResult(
                    status="success",
                    doc_id=doc_id,
                    message=f"Document {doc_id} is already active.",
                    status_code=200,
                    file_path=doc_status_data.get("file_path"),
                )
            ...
            # Helper to reactivate by setting isDeleted=False
            # We don't have explicit "restore" method, but we can use upsert or direct modification?
            # Soft delete uses a method. Reactivation could use `soft_delete(id, deleted=False)`?
            # No, soft_delete implies True.
            # We need `upsert` with `isDeleted: False`.
            
            # 2. Reactivate doc status
            await self.doc_status.soft_delete([doc_id], is_deleted=False)

            # 3. Reactivate full_docs 
            await self.full_docs.soft_delete([doc_id], is_deleted=False)
            
            # 4. Reactivate chunks
            chunk_ids = set(doc_status_data.get("chunks_list", []))
            if chunk_ids:
                chunk_id_list = list(chunk_ids)
                await self.text_chunks.soft_delete(chunk_id_list, is_deleted=False)
                await self.chunks_vdb.soft_delete(chunk_id_list, is_deleted=False)
                
                # 5. Reactivate entities
                doc_entities_data = await self.full_entities.get_by_id(doc_id)
                if doc_entities_data and "entity_names" in doc_entities_data:
                    entity_names = doc_entities_data["entity_names"]
                    await self.chunk_entity_relation_graph.soft_remove_nodes(entity_names, is_deleted=False)
                    
                    # entities_vdb uses hashed IDs
                    entity_vdb_ids = [compute_mdhash_id(name, prefix="ent-") for name in entity_names]
                    await self.entities_vdb.soft_delete(entity_vdb_ids, is_deleted=False)
                    
                    # Also reactivate in entity_chunks KV storage
                    await self.entity_chunks.soft_delete(entity_names, is_deleted=False)
                
                # 6. Reactivate relations
                doc_relations_data = await self.full_relations.get_by_id(doc_id)
                if doc_relations_data and "relation_pairs" in doc_relations_data:
                    relation_pairs = [tuple(p) for p in doc_relations_data["relation_pairs"]]
                    await self.chunk_entity_relation_graph.soft_remove_edges(relation_pairs, is_deleted=False)
                    
                    rel_ids = [compute_mdhash_id(src + tgt, prefix="rel-") for src, tgt in relation_pairs]
                    await self.relationships_vdb.soft_delete(rel_ids, is_deleted=False)

                    # Also reactivate in relation_chunks KV storage
                    relation_keys = [make_relation_chunk_key(src, tgt) for src, tgt in relation_pairs]
                    await self.relation_chunks.soft_delete(relation_keys, is_deleted=False)
            
            return DeletionResult(
                status="success",
                doc_id=doc_id,
                message=f"Document {doc_id} reactivated successfully.",
                status_code=200,
                file_path=file_path,
            )

         except Exception as e:
            logger.error(f"Reactivation failed for {doc_id}: {e}")
            return DeletionResult(
                status="fail",
                doc_id=doc_id,
                message=str(e),
    ...
    async def aactive_by_doc_id(
        self, doc_id: str
    ) -> DeletionResult:
        """Reactivate a soft-deleted document and its associated data.

        This method actively sets isDeleted=False for the document, its chunks,
        and associated entities/relationships.

        Args:
            doc_id (str): The unique identifier of the document to reactivate.
        ...
        async with pipeline_status_lock:
            if not pipeline_status.get("busy", False):
                ...
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": "Single document reactivation",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": 1,
                        "batchs": 1,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting reactivation for document: {doc_id}",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting reactivation for document: {doc_id}"
                ]
            else:
                return DeletionResult(
                    status="not_allowed",
                    doc_id=doc_id,
                    message=f"Reactivation not allowed: pipeline is busy",
                    status_code=403,
                    file_path=None,
                )
        ...
        try:
            async with pipeline_status_lock:
                 logger.info(f"Reactivating document {doc_id}")
                 pipeline_status["latest_message"] = f"Reactivating document {doc_id}"
            
            result = await self._aactive_doc_internal(doc_id)
            ...
            if we_acquired_pipeline:
                 async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    logger.info(f"Reactivation process completed for {doc_id}")
    ...
    async def adelete_by_table_name(
        ...
        **Important**: This method requires Milvus storage for chunks_vdb with table_name
        ...
        **Concurrency Control**: This method acquires the pipeline to ensure exclusive access
        ...
        Returns:
            dict: A dictionary containing:
                ...
                - table_name (str): The table_name that was targeted for deletion
                - total_docs (int): Total number of documents found with this table_name
        ...
        # Validate that chunks_vdb is Milvus storage
        if not isinstance(self.chunks_vdb, MilvusVectorDBStorage):
            return {
                ...
                "deleted_docs": [],
                "failed_docs": [],
                "message": f"delete_by_table_name requires Milvus storage for chunks. "
        ...
        try:
            # Step 1: Get all doc_ids that have chunks with this table_name
            logger.info(f"[DELETE BY TABLE] Querying documents with table_name='{table_name}'")
            doc_ids = await self.chunks_vdb.get_doc_ids_by_table_name(table_name)

            if not doc_ids:
                logger.info(f"[DELETE BY TABLE] No documents found with table_name='{table_name}'")
                return {
                    "status": "not_found",
                    "table_name": table_name,
                    "total_docs": 0,
                    "deleted_docs": [],
                    "failed_docs": [],
                    "message": f"No documents found with table_name='{table_name}'",
                }

            total_docs = len(doc_ids)
            logger.info(f"[DELETE BY TABLE] Found {total_docs} documents with table_name='{table_name}'")

            # Step 2: Acquire pipeline for batch deletion
            async with pipeline_status_lock:
                if pipeline_status.get("busy", False):
                    return {
                        "status": "not_allowed",
                ...
                # Set pipeline status for deletion job
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": f"Deleting {total_docs} Documents by table_name",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": total_docs,
                        "batchs": total_docs,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting deletion for table_name='{table_name}'",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting deletion for table_name='{table_name}' ({total_docs} documents)"
                ]
                if delete_llm_cache:
                    pipeline_status["history_messages"].append(
            ...
            try:
                ...
                for i, doc_id in enumerate(doc_ids, 1):
                    ...
                    async with pipeline_status_lock:
                        ...
                        logger.info(f"[DELETE BY TABLE] {progress_msg}")
                    ...
                    try:
                        ...
                        if result.status == "success":
                            ...
                            logger.info(f"[DELETE BY TABLE] {success_msg}")
                            async with pipeline_status_lock:
                            ...
                            logger.warning(f"[DELETE BY TABLE] {fail_msg}")
                            async with pipeline_status_lock:
                        ...
                        logger.error(f"[DELETE BY TABLE] {error_msg}")
                        async with pipeline_status_lock:
                            pipeline_status["history_messages"].append(error_msg)
                ...
                if len(deleted_docs) == total_docs:
                    status = "success"
                    message = f"Successfully deleted all {total_docs} documents with table_name='{table_name}'"
                elif len(deleted_docs) > 0:
                    status = "partial_success"
                    message = (
                        f"Partially deleted documents with table_name='{table_name}': "
                        f"{len(deleted_docs)}/{total_docs} succeeded, {len(failed_docs)} failed"
                    )
                else:
                    status = "failure"
                    message = f"Failed to delete any documents with table_name='{table_name}'"

                logger.info(f"[DELETE BY TABLE] {message}")
                ...
                # Always release pipeline
                async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    completion_msg = f"Deletion by table_name completed: {len(deleted_docs)}/{total_docs} documents deleted"
                    pipeline_status["latest_message"] = completion_msg
                    pipeline_status["history_messages"].append(completion_msg)
                    logger.info(f"[DELETE BY TABLE] {completion_msg}")

        except Exception as e:
            error_message = f"Error during delete_by_table_name for '{table_name}': {e}"
            logger.error(f"[DELETE BY TABLE] {error_message}")
            logger.error(traceback.format_exc())
            return {
                "status": "failure",
                "table_name": table_name,
    ...
    def delete_by_table_name(
        ...
        This is a synchronous wrapper around adelete_by_table_name.
        See adelete_by_table_name for detailed documentation.
    ...
    async def asoft_delete_by_table_name(
        self,
        table_name: str,
    ) -> dict:
        """Soft delete all documents with a specific table_name.
        
        Args:
            table_name (str): The table_name value to filter by.
            
        Returns:
            dict: Results of the soft deletion operation.
        """
        from lightrag.kg.milvus_impl import MilvusVectorDBStorage
        ...
        if not isinstance(self.chunks_vdb, MilvusVectorDBStorage):
            return {
                ...
                "deleted_docs": [],
                "failed_docs": [],
                "message": f"soft_delete_by_table_name requires Milvus storage for chunks. Current storage type: {type(self.chunks_vdb).__name__}",
            }
        ...
        async with pipeline_status_lock:
            if pipeline_status.get("busy", False):
                return {
                    ...
                    "deleted_docs": [],
                    "failed_docs": [],
                    "message": "Cannot soft delete documents while pipeline is busy",
                }

        try:
            logger.info(f"[SOFT DELETE BY TABLE] Querying documents with table_name='{table_name}'")
            doc_ids = await self.chunks_vdb.get_doc_ids_by_table_name(table_name)

            if not doc_ids:
                logger.info(f"[SOFT DELETE BY TABLE] No documents found with table_name='{table_name}'")
                return {
                    "status": "not_found",
                    "table_name": table_name,
                    "total_docs": 0,
            ...
            total_docs = len(doc_ids)
            logger.info(f"[SOFT DELETE BY TABLE] Found {total_docs} documents")
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": f"Soft Deleting {total_docs} Documents by table_name",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": total_docs,
                        "batchs": total_docs,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting soft deletion for table_name='{table_name}'",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting soft deletion for table_name='{table_name}' ({total_docs} documents)"
                ]
            ...
            try:
                for i, doc_id in enumerate(doc_ids, 1):
                    async with pipeline_status_lock:
                        if pipeline_status.get("cancellation_requested", False):
                            cancel_msg = f"Soft deletion cancelled by user at document {i}/{total_docs}"
                            logger.info(cancel_msg)
                            pipeline_status["latest_message"] = cancel_msg
                            pipeline_status["history_messages"].append(cancel_msg)
                            for remaining_doc_id in doc_ids[i - 1:]:
                        ...
                        progress_msg = f"Soft deleting document {i}/{total_docs}: {doc_id}"
                        logger.info(f"[SOFT DELETE BY TABLE] {progress_msg}")
                        pipeline_status["cur_batch"] = i
                        pipeline_status["latest_message"] = progress_msg
                        pipeline_status["history_messages"].append(progress_msg)
                    ...
                    try:
                        ...
                        if result.status == "success":
                            deleted_docs.append(doc_id)
                            success_msg = f"Successfully soft deleted document {i}/{total_docs}: {doc_id}"
                            async with pipeline_status_lock:
                                pipeline_status["history_messages"].append(success_msg)
                        else:
                            failed_docs.append({"doc_id": doc_id, "error": result.message})
                            fail_msg = f"Failed to soft delete document {i}/{total_docs}: {doc_id} - {result.message}"
                            logger.warning(f"[SOFT DELETE BY TABLE] {fail_msg}")
                            async with pipeline_status_lock:
                                pipeline_status["history_messages"].append(fail_msg)
                    
                    except Exception as doc_error:
                        failed_docs.append({"doc_id": doc_id, "error": str(doc_error)})
                        error_msg = f"Error soft deleting document {i}/{total_docs}: {doc_id} - {str(doc_error)}"
                        logger.error(f"[SOFT DELETE BY TABLE] {error_msg}")
                        async with pipeline_status_lock:
                            pipeline_status["history_messages"].append(error_msg)

                if len(deleted_docs) == total_docs:
                    status = "success"
                    message = f"Successfully soft deleted all {total_docs} documents with table_name='{table_name}'"
                elif len(deleted_docs) > 0:
                    status = "partial_success"
                    message = f"Partially soft deleted documents with table_name='{table_name}': {len(deleted_docs)}/{total_docs} succeeded"
                else:
                    status = "failure"
                ...
                async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    completion_msg = f"Soft deletion by table_name completed"
                    pipeline_status["latest_message"] = completion_msg
                    pipeline_status["history_messages"].append(completion_msg)

        except Exception as e:
            error_msg = f"Error during soft_delete_by_table_name for '{table_name}': {e}"
            logger.error(f"[SOFT DELETE BY TABLE] {error_msg}")
            return {
                "status": "failure",
                "table_name": table_name,
    ...
    async def asoft_delete_by_file_id(
        self,
        file_id: str,
    ) -> dict:
        """Soft delete all documents with a specific file_id.
        
        Args:
            file_id (str): The file_id value to filter by.
        ...
        if not isinstance(self.chunks_vdb, MilvusVectorDBStorage):
            return {
                ...
                "deleted_docs": [],
                "failed_docs": [],
                "message": f"soft_delete_by_file_id requires Milvus storage for chunks.",
            }
        ...
        try:
            logger.info(f"[SOFT DELETE BY FILE_ID] Querying documents with file_id='{file_id}'")
            doc_ids = await self.chunks_vdb.get_doc_ids_by_file_id(file_id)

            if not doc_ids:
                logger.info(f"[SOFT DELETE BY FILE_ID] No documents found with file_id='{file_id}'")
                return {
                    "status": "not_found",
                    "file_id": file_id,
                    "total_docs": 0,
            ...
            total_docs = len(doc_ids)
            logger.info(f"[SOFT DELETE BY FILE_ID] Found {total_docs} documents")
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": f"Soft Deleting {total_docs} Documents by file_id",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": total_docs,
                        "batchs": total_docs,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting soft deletion for file_id='{file_id}'",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting soft deletion for file_id='{file_id}' ({total_docs} documents)"
                ]
            ...
            try:
                for i, doc_id in enumerate(doc_ids, 1):
                    async with pipeline_status_lock:
                        ...
                        progress_msg = f"Soft deleting document {i}/{total_docs}: {doc_id}"
                        logger.info(f"[SOFT DELETE BY FILE_ID] {progress_msg}")
                        pipeline_status["cur_batch"] = i
                        pipeline_status["latest_message"] = progress_msg
                        pipeline_status["history_messages"].append(progress_msg)
                    ...
                    try:
                        ...
                        if result.status == "success":
                            ...
                            failed_docs.append({"doc_id": doc_id, "error": result.message})
                            fail_msg = f"Failed to soft delete document {i}/{total_docs}: {doc_id} - {result.message}"
                            logger.warning(f"[SOFT DELETE BY FILE_ID] {fail_msg}")
                            async with pipeline_status_lock:
                                pipeline_status["history_messages"].append(fail_msg)
                        ...
                        failed_docs.append({"doc_id": doc_id, "error": str(doc_error)})
                        error_msg = f"Error soft deleting document {i}/{total_docs}: {doc_id} - {str(doc_error)}"
                        logger.error(f"[SOFT DELETE BY FILE_ID] {error_msg}")
                        async with pipeline_status_lock:
                            pipeline_status["history_messages"].append(error_msg)

                if len(deleted_docs) == total_docs:
                    status = "success"
                    message = f"Successfully soft deleted all {total_docs} documents with file_id='{file_id}'"
                elif len(deleted_docs) > 0:
                    status = "partial_success"
                    message = f"Partially soft deleted documents with file_id='{file_id}': {len(deleted_docs)}/{total_docs} succeeded"
                else:
                    status = "failure"
                ...
                async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    completion_msg = f"Soft deletion by file_id completed"
                    pipeline_status["latest_message"] = completion_msg
                    pipeline_status["history_messages"].append(completion_msg)

        except Exception as e:
            error_msg = f"Error during soft_delete_by_file_id for '{file_id}': {e}"
            logger.error(f"[SOFT DELETE BY FILE_ID] {error_msg}")
            return {
                "status": "failure",
                "file_id": file_id,
    ...
    async def aactive_by_file_id(
        self,
        file_id: str,
    ) -> dict:
        """Reactivate all documents with a specific file_id.
        
        Args:
            file_id (str): The file_id value to filter by.
            
        Returns:
            dict: Results of the reactivation operation.
        """
        from lightrag.kg.milvus_impl import MilvusVectorDBStorage
        ...
        if not isinstance(self.chunks_vdb, MilvusVectorDBStorage):
            return {
                ...
                "deleted_docs": [],
                "failed_docs": [],
                "message": f"active_by_file_id requires Milvus storage for chunks.",
            }
        ...
        try:
            # Note: We need to find even DELETED documents. 
            # get_doc_ids_by_file_id needs to support include_deleted=True or we need another way.
            # MilvusVectorDBStorage.get_doc_ids_by_file_id queries by scalar filtering. 
            # If we used soft_delete, the 'isDeleted' field is set to True.
            # Does get_doc_ids_by_file_id filter by isDeleted?
            # Let's check MilvusVectorDBStorage.get_doc_ids_by_file_id implementation.
            # It likely does NOT filter by isDeleted unless specified.
            # If it DOES filter, we can't find them to reactivate!
            ...
            total_docs = len(doc_ids)
            logger.info(f"[REACTIVATE BY FILE_ID] Found {total_docs} documents")
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status.update(
                    {
                        "busy": True,
                        "job_name": f"Reactivating {total_docs} Documents by file_id",
                        "job_start": datetime.now(timezone.utc).isoformat(),
                        "docs": total_docs,
                        "batchs": total_docs,
                        "cur_batch": 0,
                        "request_pending": False,
                        "cancellation_requested": False,
                        "latest_message": f"Starting reactivation for file_id='{file_id}'",
                    }
                )
                pipeline_status["history_messages"][:] = [
                    f"Starting reactivation for file_id='{file_id}' ({total_docs} documents)"
                ]

            deleted_docs = [] # reused name for success list
            failed_docs = []
            ...
            try:
                for i, doc_id in enumerate(doc_ids, 1):
                    async with pipeline_status_lock:
                        if pipeline_status.get("cancellation_requested", False):
                            cancel_msg = f"Reactivation cancelled by user at document {i}/{total_docs}"
                            logger.info(cancel_msg)
                            pipeline_status["latest_message"] = cancel_msg
                            pipeline_status["history_messages"].append(cancel_msg)
                            for remaining_doc_id in doc_ids[i - 1:]:
                        ...
                        progress_msg = f"Reactivating document {i}/{total_docs}: {doc_id}"
                        logger.info(f"[REACTIVATE BY FILE_ID] {progress_msg}")
                        pipeline_status["cur_batch"] = i
                        pipeline_status["latest_message"] = progress_msg
                        pipeline_status["history_messages"].append(progress_msg)
                    ...
                    try:
                        ...
                        if result.status == "success":
                            deleted_docs.append(doc_id)
                            success_msg = f"Successfully reactivated document {i}/{total_docs}: {doc_id}"
                            async with pipeline_status_lock:
                                pipeline_status["history_messages"].append(success_msg)
                        else:
                            failed_docs.append({"doc_id": doc_id, "error": result.message})
                            fail_msg = f"Failed to reactivate document {i}/{total_docs}: {doc_id} - {result.message}"
                            logger.warning(f"[REACTIVATE BY FILE_ID] {fail_msg}")
                            async with pipeline_status_lock:
                                pipeline_status["history_messages"].append(fail_msg)
                    
                    except Exception as doc_error:
                        failed_docs.append({"doc_id": doc_id, "error": str(doc_error)})
                        error_msg = f"Error reactivating document {i}/{total_docs}: {doc_id} - {str(doc_error)}"
                        logger.error(f"[REACTIVATE BY FILE_ID] {error_msg}")
                        async with pipeline_status_lock:
                            pipeline_status["history_messages"].append(error_msg)

                if len(deleted_docs) == total_docs:
                    status = "success"
                    message = f"Successfully reactivated all {total_docs} documents with file_id='{file_id}'"
                elif len(deleted_docs) > 0:
                    status = "partial_success"
                    message = f"Partially reactivated documents with file_id='{file_id}': {len(deleted_docs)}/{total_docs} succeeded"
                else:
                    status = "failure"
                ...
                async with pipeline_status_lock:
                    pipeline_status["busy"] = False
                    pipeline_status["cancellation_requested"] = False
                    completion_msg = f"Reactivation by file_id completed"
                    pipeline_status["latest_message"] = completion_msg
                    pipeline_status["history_messages"].append(completion_msg)

        except Exception as e:
            error_msg = f"Error during active_by_file_id for '{file_id}': {e}"
            logger.error(f"[REACTIVATE BY FILE_ID] {error_msg}")
            return {
                "status": "failure",
                "file_id": file_id,
